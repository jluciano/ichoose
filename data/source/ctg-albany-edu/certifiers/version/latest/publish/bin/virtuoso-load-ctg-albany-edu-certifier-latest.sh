#!/bin/bash
#
# run publish/bin/virtuoso-load-ctg-albany-edu-certifier-latest.sh
# from source/ctg-albany-edu/certifier/version/latest/
#
# graph was http://ichoose.tw.rpi.edu/source/ctg-albany-edu/dataset/certifier/version/latest during conversion
...
if [ -e 'publish/bin/ln-to-www-root-ctg-albany-edu-certifier-latest.sh' ]; then
   # Make sure that the file we will load from the web is published
   publish/bin/ln-to-www-root-ctg-albany-edu-certifier-latest.sh
fi

base="${CSV2RDF4LOD_BASE_URI_OVERRIDE:-$CSV2RDF4LOD_BASE_URI}"
graph="$base/source/ctg-albany-edu/dataset/certifier/version/latest"
metaGraph="$graph"
if [ "$1" == "--sample" ]; then
   layerSlug="raw"
   sampleGraph="$graph/conversion/$layerSlug/subset/sample"
   sampleURL="$base/source/ctg-albany-edu/file/certifier/version/latest/conversion/ctg-albany-edu-certifier-latest.raw.sample.ttl"
   ...
   sampleGraph="$graph/conversion/$layerSlug/subset/sample"
   sampleURL="$base/source/ctg-albany-edu/file/certifier/version/latest/conversion/ctg-albany-edu-certifier-latest.e1.sample.ttl"
   echo pvload.sh $sampleURL -ng $sampleGraph
   ${CSV2RDF4LOD_HOME}/bin/util/pvload.sh $sampleURL -ng $sampleGraph

   exit 1
elif [[ "$1" == "--meta" && -e 'publish/ctg-albany-edu-certifier-latest.void.ttl' ]]; then
   metaURL="$base/source/ctg-albany-edu/file/certifier/version/latest/conversion/ctg-albany-edu-certifier-latest.void.ttl"
...
if [[ "$1" == "--unversioned" || "$1" == "--abstract" ]]; then
   ...
   graph="$base/source/ctg-albany-edu/dataset/certifier"
...
dump='publish/ctg-albany-edu-certifier-latest.nt'
url='http://ichoose.tw.rpi.edu/source/ctg-albany-edu/file/certifier/version/latest/conversion/ctg-albany-edu-certifier-latest.nt'
if [ -e $dump ]; then
...
dump='publish/ctg-albany-edu-certifier-latest.ttl'
url='http://ichoose.tw.rpi.edu/source/ctg-albany-edu/file/certifier/version/latest/conversion/ctg-albany-edu-certifier-latest.ttl'
if [ -e $dump ]; then
   echo pvload.sh $url -ng $graph
   ${CSV2RDF4LOD_HOME}/bin/util/pvload.sh $url -ng $graph
   exit 1
...
dump='publish/ctg-albany-edu-certifier-latest.rdf'
url='http://ichoose.tw.rpi.edu/source/ctg-albany-edu/file/certifier/version/latest/conversion/ctg-albany-edu-certifier-latest.rdf'
if [ -e $dump ]; then
   ${CSV2RDF4LOD_HOME}/bin/util/pvload.sh $url -ng $graph
   exit 1
elif [ -e $dump.gz ]; then
...
#3> <> prov:wasAttributedTo <http://ichoose.tw.rpi.edu/id/csv2rdf4lod/f18b2d4849f3e7625df42d53a5711000> .
#3> <> prov:generatedAtTime "2013-08-03T23:59:08+00:00"^^xsd:dateTime .
#3> <http://ichoose.tw.rpi.edu/id/csv2rdf4lod/f18b2d4849f3e7625df42d53a5711000> foaf:name "convert-certifier.sh" .
